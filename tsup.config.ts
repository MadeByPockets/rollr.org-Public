...
function discoverEntries() {
  ...
  // Only add nested components as entries if they have an index file
  // But for the root components, we use the src/*.ts entries generated by update-barrels.mjs
  addFolderIndexEntries('src/components', '', (relDir) => {
    if (!relDir) return ''; // skip root components/index
    const aliased = aliasComponents(relDir);
    // If it's a top-level component, skip it here because we'll add it via src/*.ts
    if (!aliased.includes('/')) return '';
    return aliased;
  });

  // Add the root component entries from src/*.ts
...
export default defineConfig({
  ...
  treeshake: true,
  onSuccess: async () => {
    // Optional: add any post-build steps here if needed
  },
});
